...
ocrpasswd = "BACB901A912CCCFAFF0CA2D985AF6A9F"
# VcodeInit 初始换引擎函数 只有一个参数 为引擎初始化密码 失败返回-1 此函数只需调用一次 切勿多次调用 。
dll = windll.LoadLibrary('E:/PyCharm/CatPackages/resources/dll/CaptchaOCR.dll')
load_ocr = dll.VcodeInit
load_ocr.argtypes = [c_char_p]
load_ocr.restypes = c_int
index = load_ocr(ocrpasswd)


def resolve(img_string):
    img_buffer = create_string_buffer(img_string)
    # 申请接收识别结果的缓冲区  一定要申请
    ret_buffer = create_string_buffer(10)
